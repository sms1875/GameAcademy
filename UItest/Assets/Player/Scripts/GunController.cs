...
public class GunController : MonoBehaviour
{
    ...
    void Start()
    {
     ...
     //무기 추가
        for (int i = 0; i < playerController.currentGunList.Length; i++)
        {
            gunDictionary.Add(playerController.currentGunList[i].gunName, playerController.currentGunList[i]);
        }
        init();
    ...
    {    
        //첫번째 무기 장착
        currentGun = playerController.currentGunList[0];
    ...
    void Update()
    {
        ...
        {
            if (Input.GetKeyDown(KeyCode.Alpha1) && currentGun.name != playerController.currentGunList[0].name)
                StartCoroutine(ChangeWeaponCoroutine("GUN", playerController.currentGunList[0].name));
            else if (Input.GetKeyDown(KeyCode.Alpha2) && currentGun.name != playerController.currentGunList[1].name)
                StartCoroutine(ChangeWeaponCoroutine("GUN", playerController.currentGunList[1].name));
        }
    }
    ...
    {   
        isFire = true;
        if (!playerController.GetRun() && weaponAnim.GetCurrentAnimatorStateInfo(0).IsName("armature_" + currentGun.name + "_Idle"))//달리기판단,Idle에서만 사격
        {
    ...
    */
        ...
        public void setGunAnimation()
    {
        ...
        weaponAnim = playerController.anim;
